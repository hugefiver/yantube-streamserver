...
use {
    super::{common::Common, define, define::SessionType, errors::SessionError},
    crate::{
        chunk::{
            define::CHUNK_SIZE,
...
impl ClientSession {
    pub fn new(
        stream: TcpStream, client_type: ClientType, raw_domain_name: String, app_name: String,
        raw_stream_name: String, event_producer: StreamHubEventSender, gop_num: usize,
    ) -> Self {
    ...
    pub async fn process_messages(
        &mut self, msg: &mut RtmpMessageData, timestamp: &u32,
    ) -> Result<(), SessionError> {
        match msg {
    ...
    pub async fn on_amf0_command_message(
        &mut self, command_name: &Amf0ValueType, transaction_id: &Amf0ValueType,
        command_object: &Amf0ValueType, others: &mut Vec<Amf0ValueType>,
    ) -> Result<(), SessionError> {
        log::info!("[C <- S] on_amf0_command_message...");
    ...
    pub async fn send_delete_stream(
        &mut self, transaction_id: &f64, stream_id: &f64,
    ) -> Result<(), SessionError> {
    ...
    pub async fn send_publish(
        &mut self, transaction_id: &f64, stream_name: &String, stream_type: &String,
    ) -> Result<(), SessionError> {
        let mut netstream = NetStreamWriter::new(Arc::clone(&self.io));
        netstream
    ...
    pub async fn send_play(
        &mut self, transaction_id: &f64, stream_name: &String, start: &f64, duration: &f64,
        reset: &bool,
    ) -> Result<(), SessionError> {
        let mut netstream = NetStreamWriter::new(Arc::clone(&self.io));
        netstream
    ...
    pub async fn on_status(
        &mut self, obj: &IndexMap<String, Amf0ValueType>,
    ) -> Result<(), SessionError> {
